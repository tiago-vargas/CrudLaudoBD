...
public class LaboratorioDAO extends ConexaoDB {
    ...
    private static final String SELECT_LABORATORIO_BY_ID = "SELECT * FROM laboratorio WHERE id = ?";
    ...
    private static final String BUSCAR_POR_NOME_FANTASIA_SQ = "DELETE FROM laboratorio WHERE nome_fantasia = ?;";
    ...
    public Laboratorio insert(Laboratorio entidade) {
        try (PreparedStatement preparedStatement = prepararSQL(INSERT_LABORATORIO_SQL,
            ...
            preparedStatement.setString(2, entidade.getCrbm());
            preparedStatement.setString(2, entidade.getNomeFantasia());
    ...
    public Laboratorio findByNomeFantasia(String nomeFantasia) {
        Laboratorio entidade = null;
        try (PreparedStatement preparedStatement = prepararSQL(BUSCAR_POR_NOME_FANTASIA_SQ)) {
            preparedStatement.setString(1, nomeFantasia);
            ResultSet rs = preparedStatement.executeQuery();

            while (rs.next()) {
                entidade = new Laboratorio(rs.getLong("id"), rs.getString("nome_fantasia"));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
    ...
    public void updateLaboratorio(Laboratorio entidade) throws SQLException {
        try (PreparedStatement statement = prepararSQL(UPDATE_LABORATORIO_SQL)) {
            ...
            statement.setString(2, entidade.getCrbm());
            statement.setString(2, entidade.getNomeFantasia());
